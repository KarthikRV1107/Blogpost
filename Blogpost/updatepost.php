...
if ($authorResult && $authorResult->num_rows > 0) {
    ...
    if ($post['user_id'] == $_SESSION['user_id']) {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($errors)) {
                $updateSql = "UPDATE posts SET title = ?, content = ?, user_id = ?, image_path = ? WHERE id = ?";
                $updateStmt = $conn->prepare($updateSql);
                $updateStmt->bind_param("ssisi", $title, $content, $user_id, $image_path, $post_id);
    ...
    <body>
        <div class="denied-container">
            <h1><i class="fas fa-ban"></i> Access Denied</h1>
            <p>Your does not have permission to Edit this post.</p>
            <a href="displaypost.php">Return Back</a>
        </div>
    </body>
    </html>';
        exit();
